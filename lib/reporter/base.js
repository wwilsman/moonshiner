...
export class Reporter {
  ...
  summarize(data, {
    ...
    for (let { test, error } of this.errors(data)) {
      ...
      if (test.parent) {
        summary += (function title({ name, depth, parent }) {
      ...
      if (error instanceof Error) {
        ...
        error = `${error.name}: ${error.message.trim()}\n${stack}`;
      }
    ...
    let duration = data.duration >= 1000 ? style('dim', ` (${formatTime(data.duration)})`) : '';

    summary += `\n${symbols['test:end'] ?? ''}${style('white', 'Summary')}\n`;
