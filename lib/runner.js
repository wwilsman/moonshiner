...
function runTestSuite(suite) {
  return suite.emit('run', 'suite', suite, async suite => {
    ...
    let skipAll = ({ tests, suites }) =>
      (!tests || tests.every(t => t.skip)) &&
      (!suites || suites.every(skipAll));
    if (!skip) skip = skipAll(suite);

    let runAll = async (runnables = [], ctx) => {
      for (let runnable of runnables) {
        if (!runnable.skip && !ctx.skip) await runnable.run();
        if (!runnable.error) continue;
        ctx.error ??= runnable.error;
        suite.success = false;
        return;
      }
    };

    if (!skip) await runAll(suite.beforeAlls, suite);
    ...
    for (let inner of suites ?? []) {
      ...
      if (suite.error || (skip && !inner.skip)) inner.skip = true;
      if (!(await inner.run()).success) suite.success = false;
    }

    if (!skip) await runAll(suite.afterAlls, suite);
    suite.success ??= true;
    return suite;
  });
}
