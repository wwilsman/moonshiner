...
export class TestRunner {
  ...
  run(test, options) {
    ...
    meta.signal.addEventListener('abort', () => {
      if (meta.signal.reason) info.aborted = meta.signal.reason;
      test.emit('test:abort', meta.signal.reason, test);
    });

    meta.promise = this.#runTest(test, meta).finally(() => {
      if (test.depth || !test.debug) {
        signal?.removeEventListener('abort', abort);
        meta.controller.abort(info.error);
      }
    });
  ...
  abort(test, reason) {
    this.#tests.get(test)?.controller.abort(reason);
  }

  async #runTest(test, { info, controller }) {
    try {
      await test.configure();
      if (!test.depth) await test.emit('test:prepare');
      if (test.type === 'suite') await test.ready();
      if (!test.depth) await test.emit('test:start');
      await test.emit('test:ready', info);

      if (test.type === 'test' && test.skip) {
        info = this.#collectResults(test, info);
        await test.emit('test:skip', info);
        return info;
      }
    ...
    info = this.#collectResults(test, info);
    await test.emit(info.pass ? 'test:pass' : 'test:fail', info);
    if (!test.depth) await test.emit('test:end', info);
    return info;
  }
  ...
  async #raceTimeout(test) {
    ...
    return await Promise.race([
      ...
      new Promise((_, reject) => {
        ...
        context.timeout = ms => startTimeout(test.timeout = ms, reject);
        startTimeout(test.timeout, reject);
      })
    ]).finally(() => {
      clearTimeout(timeout);
    });
  ...
  #collectResults(test, info) {
    ...
    info.total = test.children.reduce((total, test) => {
      ...
      return {
        ...
        count: total.count + (i?.total?.count ?? 0) +
          (test.type === 'test' ? 1 : 0)
      };
    }, { passing: 0, failing: 0, skipped: 0, count: 0 });
