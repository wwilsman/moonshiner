...
export class TestRunner {
  ...
  run(test, options) {
    if (!this.#running) this.#tests.clear();
    ...
    meta.promise = this.#runTest(test, meta).finally(() => {
      ...
      if (!test.depth) this.#running = false;
    });
  ...
  async #runTest(test, { info, controller }) {
    ...
    for (let hook of [].concat(
      test.hooks.before,
      test.hooks.beforeEach,
      test.hooks.afterEach,
      test.hooks.after
    )) this.#tests.delete(hook);
  ...
  async #raceTimeout(test) {
    ...
    let promise = new Promise((resolve, reject) => {
      ...
      signal.addEventListener('abort', abort = () => reject(signal.reason));
      ...
      Promise.resolve()
        ...
        .then(() => test.fn?.length < 2
          ...
          : test.fn?.call(context, context, e => e ? reject(e) : resolve()))
        .then(result => test.fn?.length < 2 ? resolve(result) : promise)
  ...
  #collectResults(test, info) {
    ...
    if (!test.parent && !info.total.count)
      info.error ??= 'Tests not found';
