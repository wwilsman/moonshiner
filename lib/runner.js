...
function runWithin(fn, timeout = 2000) {
  if (timeout === false) return fn();
  let resolve, reject, deferred = new Promise((...a) => [resolve, reject] = a);
  setTimeout(reject, timeout, new Error(`Timeout exceeded (${timeout}ms)`));
...
function createRunnable(options, run) {
  ...
  runnable.timeout = ms => ms ? (timeout = ms) :
    timeout ?? runnable.suite?.timeout();
