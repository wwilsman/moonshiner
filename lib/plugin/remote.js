...
export class RemoteSync {
  ...
  apply(test) {
    ...
    test.on('test:configure', async ({ config }) => {
      ...
      await this.#remote.on((event, data) => {
        return test.trigger('remote:event', { event, data });
      });
    });

    test.on('test:ready', async () => {
    ...
    test.on('test:abort', async ({ test, aborted }) => {
      for (let [connection] of this.#testConnections.get(test.id) ?? [])
        connection.send('test:abort', { test, aborted });
    });
    ...
    test.on('remote:event', ({ event, data }) => {
      ...
      if (event === 'test:abort')
        test.lookup(data.test.id)?.abort(data.aborted);
    });
  ...
  #connect(connection) {
    ...
    connection.on('test:abort', ({ test, aborted }) => {
      if (!test.depth) this.#test.abort(aborted);
    });
    ...
    connection.on((event, data) => {
      return this.#test.trigger('connection:event', { event, data });
    });
