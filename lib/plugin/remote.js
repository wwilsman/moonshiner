...
export class RemoteSync {
  ...
  apply(test) {
    ...
    test.on('browser:navigation', ({ url }) => {
      this.#navigated = url;
    });
  ...
  #connect(remote) {
    ...
    remote.on('close', () => {
      if (running) this.#test.abort(new Error(
        'Remote connection closed unexpectedly' + (this.#navigated
          ? `\n\nBrowser navigated to: ${this.#navigated}`
          : '\n\nCommon causes: uncaught error, network issue, crash'
        )));
