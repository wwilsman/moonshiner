...
export class ScreenshotCapture {
  ...
  apply(test) {
    ...
    test.on('screenshot:capture', async ({ data, ...options }) => {
      ...
      if (cache.has(name)) throw new Error(`Duplicate screenshot name: ${name}`);
      ...
      if (!fs.existsSync(baseline)) await fs.promises.writeFile(baseline, data, 'base64');
      if (fs.existsSync(diff)) await fs.promises.unlink(diff);

      let match = await fs.promises.readFile(baseline, 'base64') === data;
      let result = { match };
      ...
      if ((result.match || this.#update) && fs.existsSync(comparison))
        await fs.promises.unlink(comparison);
      if ((result.match || this.#update) && fs.existsSync(diff))
        await fs.promises.unlink(diff);
      if (!(result.match || this.#update) && this.#disable !== 'throw')
