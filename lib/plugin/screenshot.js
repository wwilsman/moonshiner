...
export class Screenshot {
  ...
  apply(test) {
    test.on('screenshot:capture', async ({ group, name, data, format }) => {
      ...
      let filename = fs.existsSync(baseline) ? comparison : baseline;
      ...
      if (filename === comparison) {
        let result = await this.#compare?.(baseline, comparison, diff);
        if (result?.match) await fs.promises.unlink(comparison);
      }
    });
  }
}
