...
export class ScreenshotCapture {
  ...
  apply(test) {
    ...
    test.on('screenshot:capture', async options => {
      ...
      if (result && !(result.match || this.#update) && this.#disable !== 'throw') {
        options.error = new Error(`Screenshot comparison failed for ${options.group}`);
      }

      return options;
    });

    // Defer mismatch errors so wildcard listeners can observe the enriched payload.
    test.on(async (event, options) => {
      if (event !== 'screenshot:capture' || !options.error) return;
      throw options.error;
    }, { priority: 100 });
  }
  ...
  #capture = async options => {
    ...
    if (!fs.existsSync(options.baseline)) await fs.promises.writeFile(options.baseline, options.base64, 'base64');
    if (fs.existsSync(options.diff)) await fs.promises.unlink(options.diff);

    options.match = await fs.promises.readFile(options.baseline, 'base64') === options.base64;
    let result = { match: options.match };

    if (!result.match)
      await fs.promises.writeFile(options.comparison, options.base64, 'base64');
    if (!result.match && this.#compare)
      result = await this.#compare(options) ?? {};
    options.match = result.match ?? options.match;
    if (!result.match && this.#update)
      await fs.promises.writeFile(options.baseline, options.base64, 'base64');
    if ((options.match || this.#update) && fs.existsSync(options.comparison))
      await fs.promises.unlink(options.comparison);
    if ((options.match || this.#update) && fs.existsSync(options.diff))
      await fs.promises.unlink(options.diff);

    return { ...result, match: options.match };
  };
}
