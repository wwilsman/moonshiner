...
/**
 * @typedef {Object} ScreenshotComparison
 * @property {string} baseline - Path to the existing baseline screenshot
 * @property {string} comparison - Path to the newly captured screenshot
 * @property {string} diff - Path where a diff image can be written
 * @property {boolean} match - Initial byte-for-byte match result
 * @property {string} group - Screenshot group, typically the browser name
 * @property {string} fullname - Final screenshot filename without suffix
 * @property {string} format - Screenshot image format
 * @property {string} base64 - Captured image contents
 * @property {BaseTestContext} [test] - Test metadata associated with the screenshot
 */
 ...
 * @property {function(ScreenshotComparison): Promise<{match: boolean}>} [compare] - Custom function to compare screenshots. Receives screenshot metadata and paths. Should return {match: true} if screenshots are identical.
 */
 ...
 * @type {{
 *   (name: string, fn: TestSuiteFn): import('./test.js').TestSuite;
 *   (name: string, options: TestOptions, fn: TestSuiteFn): import('./test.js').TestSuite;
 *   only(name: string, fn: TestSuiteFn): import('./test.js').TestSuite;
 *   only(name: string, options: TestOptions, fn: TestSuiteFn): import('./test.js').TestSuite;
 *   skip(name: string, fn?: TestSuiteFn): import('./test.js').TestSuite;
 *   skip(name: string, options?: TestOptions, fn?: TestSuiteFn): import('./test.js').TestSuite;
 * }}
 */
 ...
 * Define a test case
 * @type {{
 *   (name: string, fn: TestFn): import('./test.js').Test;
 *   (name: string, options: TestOptions, fn: TestFn): import('./test.js').Test;
 *   only(name: string, fn: TestFn): import('./test.js').Test;
 *   only(name: string, options: TestOptions, fn: TestFn): import('./test.js').Test;
 *   skip(name: string, fn?: TestFn): import('./test.js').Test;
 *   skip(name: string, options?: TestOptions, fn?: TestFn): import('./test.js').Test;
 * }}
 */
 ...
 * @type {{
 *   (fn: TestFn): import('./test.js').TestHook;
 *   (options: TestOptions, fn: TestFn): import('./test.js').TestHook;
 * }}
 */
