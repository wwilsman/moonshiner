...
export function summarize(suite) {
  return [suite].reduce(function tally(sum, suite) {
    let find = (type, id) =>
      suite[`before${type}s`]?.find(h => h.id === id) ??
      suite[`after${type}s`]?.find(h => h.id === id);

    let error = src => {
      src.error.source &&= src.error.source === src.id ? src
        : find('All', src.error.source) ?? src.error.source;
      (sum.errors ??= []).push(src.error);
      return src;
    };

    if (suite.error) error(suite);

    for (let test of suite.tests ?? []) {
      if (test.error) (sum.failing ??= []).push(error(test));
