...
function buildReportTable(event, merge) {
  if (!event.client?.id || !event.data) return;
  ...
  let reports = collect(new Map(), data, (reports, meta) => {
    let entry = merge?.get(meta.id) ?? { clients: {}, reports: {} };

    if (ids.has(meta.id)) {
      entry.clients[event.client.id] = event.client;
      entry.reports[event.client.id] = {};
    }
...
function updateReportTable({ type, name, data, client }, table) {
  if (type !== 'report' || !table || !client?.id ||
    !data?.id || !table.get(data.id)) return;
  table.get(data.id).reports[client.id][name] = data;
...
function syncReport(event, table, callback) {
  ...
  return report.pending.promise.then(async () => {
    await new Promise(r => setTimeout(r, 0));
    return callback(report.data);
  });
}

export function createReporter(handler, options, state) {
  if (typeof handler !== 'function')
    [handler, options, state] = [null, handler, options];
  if (typeof options === 'function')
    [options, state] = [null, options];
  state ??= options?.state ?? {};
  let clientReports;
  ...
  return function reporter(event, next) {
    let handle = (type, name, data) => {
      ...
      if (handler) handler.call(this, evt, state);
      else options[name]?.call(this, data, state, evt);
    ...
    if (event.type === 'client' && event.name === 'ready' && options?.sync !== false)
