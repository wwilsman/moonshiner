function buildReportTable({ data, client }, table = {}, visited = new Set()) {
  if (visited.has(data)) return table;
  visited.add(data);

  if (data.id && client.name)
    (table[data.id] ??= {})[client.name] = {};
...
function updateReportTable({ type, name, data, client }, table) {
  if (type !== 'report' || !table || !data?.id || !client) return;

  table[data.id][client.name][name] = data;
  return Object.values(table[data.id]).map(c => c[name]);
}
...
export function createReporter(handler, state = {}) {
  ...
  return function reporter(event, next) {
    ...
    if (event.type === 'console' || event.type === 'report') {
      ...
      if (reports?.every(Boolean))
        handle(event.type, event.name, mergeReports(reports))
    }
