...
function serialize(data, visited = new Set()) {
  if (visited.has(data) && data.id) return data.id;
  ...
  for (let prop in data) {
    if (prop === 'ref' && data.ref.current) continue;
...
export function reporter({
  ...
  let send = (type, name, data) => connected.then(() => {
    ...
    if (globalThis?.__coverage__ && name === 'after:suite' && data.depth === 0)
      payload.data.coverage ??= globalThis.__coverage__;
    if (data.path && !payload.data.path)
      payload.data.path = data.path;
