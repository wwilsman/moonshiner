import createReporter from './base.js';

function serialize(data, visited = new Set()) {
  if (data == null || visited.has(data) || typeof data === 'function') return;
  if (typeof data !== 'object') return data;
  if (Object.getPrototypeOf(data) !== Object.prototype) return;

  let serialized = Array.isArray(data) ? [] : {};
  visited.add(data);

  if (data?.error) serialized.error = {
    message: data.error.message,
    stack: data.error.stack
  };

  for (let prop in data) {
    let val = serialize(data[prop], new Set(visited));
    if (val != null) serialized[prop] = val;
  }

  return serialized;
}

export function reporter({
  WebSocket = WebSocket,
  url = 'ws://localhost:8080',
  socket = new WebSocket(url)
} = {}) {
  let connected = new Promise((res, rej) => {
    if (socket.readyState === 1) return res();
    socket.addEventListener('open', res, { once: true });
    socket.addEventListener('error', rej, { once: true });
  });

  let send = (type, name, data) => connected.then(() => {
    socket.send(JSON.stringify({ type, name, data: serialize(data) }));
  });

  let reporter = createReporter(({ type, name, data }) => {
    return send(type, name, data);
  }, runner => {
    let run, ready = new Promise(r => (run = r));

    socket.addEventListener('message', ({ data }) => {
      if (data === 'run') return run();
    });

    runner.use(({ type, name, data }, next) => {
      if (type === 'run' && name === 'suite' && data.depth === 0)
        return send('ready').then(() => ready.then(next));
      return next();
    });
  });

  return Object.assign(reporter, { send });
}

export default reporter;
