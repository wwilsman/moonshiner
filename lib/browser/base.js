...
export class BrowserLauncher {
  ...
  async apply(test) {
    test.on('test:abort', (reason, test) => {
      if (!test.depth) this.#controller.abort(reason);
    });
    ...
    test.on('server:event', async (id, event, data) => {
      ...
      if (event === 'devtools:send' && this.devtools) {
        ...
        if (data.method === 'Page.captureScreenshot') {
          await test.emit('screenshot:capture', results.data, {
            group: this.name, ...data.meta, format: data.params?.format ?? 'png'
          });
        }
