...
export class BrowserLauncher {
  ...
  configure(config) {
    ...
    if (config.deviceScaleFactor != null)
      ...
      try {
        let webSocketUrl = await fetch(`http://localhost:${port}/json`, { method: 'get' })
          .then(r => r.json()).then(a =>
            a.find(t => t.type === 'page' && t.url === this.url) ??
            a.find(t => t.type === 'page' && !t.url.startsWith('devtools:')))
        ...
        if (devtools.bidi) {
          await devtools.send('session.subscribe', {
            events: ['browsingContext.navigationStarted']
          }).catch(() => {});

          devtools.on('browsingContext.navigationStarted', params => {
            if (params.url !== this.url)
              this.#test?.trigger('browser:navigation', { url: params.url });
          });

          await devtools.send('browsingContext.setViewport', {
            context: devtools.bidi.context,
            viewport: { width: this.width, height: this.height },
            devicePixelRatio: this.deviceScaleFactor
          }).catch(() => {});
        } else {
          await devtools.send('Page.enable', {}).catch(() => {});

          const { frameTree: { frame: mainFrame } = {} } =
            await devtools.send('Page.getFrameTree', {}).catch(() => ({}));

          devtools.on('Page.frameNavigated', params => {
            if (params.frame.id === mainFrame?.id && params.frame.url !== this.url)
              this.#test?.trigger('browser:navigation', { url: params.frame.url });
          });
