...
export class Test {
  ...
  define(property, value) {
    this.#contextProperties[property] = value;
  }
  ...
  async run({ context = {}, signal, ctx } = {}) {
    ...
    let results = { test: this, hooks: {}, children: [], start };
    ...
    if (!isTestHook)
      ctx = new TestContext(this, signal, ctx);
    ...
    let collectTotals = () => results.children.reduce((total, res) => ({
      passing: total.passing + (res.total?.passing ?? 0) +
        (res.test.type === 'test' && res.pass ? 1 : 0),
      failing: total.failing + (res.total?.failing ?? 0) +
        (res.test.type === 'test' && res.fail ? 1 : 0),
      skipped: total.skipped + (res.total?.skipped ?? 0) +
        (res.test.type === 'test' && res.skip ? 1 : 0)
    ...
    try {
      ...
      if (!this.parent)
        await this.emit('test:prepare');

      if (isTestSuite) await this.ready();

      if (!this.parent)
        await this.emit('test:start', { ...results });
      ...
      if (results.skip) {
        ...
        if (this.skip) await this.emit('test:skip', { ...results });
      ...
      Object.assign(ctx, this.#contextProperties);
      ...
      try {
        if (isTestSuite) {
          ...
          for (let child of this.children) {
            await child.run({ context, signal, ctx });
    ...
    results.end = performance.timeOrigin + performance.now();
    ...
    if (!this.parent)
      await this.emit('test:end', { ...results });
