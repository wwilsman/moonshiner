...
export class Test {
  ...
  configure(config) {
    ...
    if (config.reporter || config.reporters) {
      ...
      ).reduce((reporters, reporter) => {
        if (reporter) reporters.push(this.#reporter(reporter));
        return reporters;
      }, []);
    }
  }
  ...
  async report(type, data) {
    for (let report of this.#reporters)
      await report(type, data);
  }

  #reporter(reporter) {
    let running, next, stop;
    let events = [];
    ...
    return (type, data) => new Promise(resolve => {
      if (type === 'run:end') stop = true;
      events.push({ type, data, resolve });
      if (running) return next?.();

      running = Promise.resolve().then(async () => {
        let result, report = reporter(source());
  ...
  async run({ context = {}, signal, ctx } = {}) {
    ...
    let report = context.report ??= (event, data) => this.report(event, data);
