...
export function createTestServer({
  ...
  let runWhenReady = () => {
    ...
    runWhenReady._timeout = setTimeout(() => {
      ...
      ready = Promise.all(Array.from(clients, c => c.ready.promise));
      ready.then(() => sockets.forEach(ws => ws.send('run')));
      done = Promise.all(Array.from(clients, c => c.done.promise));
      done.then(() => once !== false && server.close());
    }, timeout);
  };
