...
export const chrome = createBrowserLauncher({
  ...
  executablePath: ({ name, platform }) => ({
    linux: path.join('chrome-linux64', 'chrome-for-testing'),
    win32: path.join('chrome-win64', 'chrome-for-testing.exe'),
    win64: path.join('chrome-win64', 'chrome-for-testing.exe'),
  ...
  launchArgs: ({ url, args, profile, headless, devtoolsPort, width, height }) => [
