  ...
  if (!fs.existsSync(executablePath)) {
    try {
      if (!fs.existsSync(downloadPath)) {
        log.write(`Downloading ${options.name}...\n`);
        ...
        await new Promise((resolve, reject) => https.get(downloadUrl, res => {
          if (res.statusCode !== 200) {
            ...
            res.on('data', chunk => (
              log.progress(`Downloading ${options.name}`, chunk.length, total)
            ));
      ...
      if (extractFile) {
        log.rewrite(`Extracting ${options.name}...\n`);
        await extractFile(downloadPath, downloadDir);
        log.rewrite(`Installed ${options.name}\n`);
      }
    } finally {
