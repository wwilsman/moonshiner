  ...
  return (url, options) => launch(async server => {
    ...
    if (browser.devtoolsPort) {
      ...
      if (browser.consoleLogs !== false) {
        devtools.on('Runtime.consoleAPICalled',
          ({ type, args }) => console[type]?.(`[${browser.name}]`,
            ...args.map(a => a.value ?? a.description)));
      }

      await devtools.send('Runtime.enable');
      await devtools.send('Runtime.evaluate', {
        expression: `window.__MOONSHINER_DEVTOOLS_URL__ = "${devtools.url}";`
      });
